...
<html>
	...
	<body>
		...
		function loadAllUsers(page,size){
			//发送请求加载题目
			$.post('<%=request.getContextPath() %>/ajaxloadexercise/ajaxexe',{pageNo:page,pageSize:size},function(data){
				 clearTable();
				var users = data.list;
				for(var i=0;i<users.length;i++){
					$('#sp3').text(users[i].testquestion_content);
					//发送请求加载选项
					$.post('<%=request.getContextPath() %>/ajaxloadexercise/ajaxopt',{testquestion_id:users[i].testquestion_id},function(data){						
